...
public class Connect4Rules implements GameRules {
	...
	// Return a new board with connect4 dimensions
	public Board newBoard() {
		return new Board(dimX, dimY);
	}
	
	// Consulta si hay empate
	// ???????? En conecta lo necesitamos?????
	...
	public boolean checkHorizontal(Board b) {
		...
		y = b.getHeight(); // Starts at bottom
		
		while((y >= 1) && (!isFormed)) 
		{	
			tilesCounter = 1; // Reset counter
			x = 1; // Starts at first position
			counter = b.getPosition(x, y); // Color of first cell on each iteration
			
			while ((x < dimX) && (!isFormed)) 
			{
				nextCounter = b.getPosition(x + 1, y);
				...
				if ((counter == nextCounter) && (counter != Counter.EMPTY)) {
					...
					tilesCounter = 1;
					counter = b.getPosition(x + 1, y); // next Cell color
				}		
	...
	public boolean checkVertical(Board b) {
		...
		{
			tilesCounter = 1; // Reset counter
			y = b.getHeight(); // Start at bottom
			counter = b.getPosition(x, y); // Color of first cell on each iteration
			
			while((y > 1) && (!isFormed))  
			{
				nextCounter = b.getPosition(x, y - 1); // take the color of row before
				...
				if ((counter == nextCounter) && (counter != Counter.EMPTY)) {
					...
					tilesCounter = 1;
					counter = b.getPosition(x, y - 1); // next Cell color
				}		
		...
		while ((y <= board.getHeight()) && !(isFormed)) {
			...
			while ((x < numIterations) && !(isFormed)) {
				...
				if ((color == nextColor) && (color != Counter.EMPTY)) {
					...
					if (tilesCounter == Resources.TILES_TO_WIN) {
						isFormed = true;
						// finished = true; En conecta 4 no usamos esto
						this.winner = color;
					}
				}
				else
			...
			while ((x > 1) && !(isFormed)) {
				...
				while ((counter < numIterations) && !(isFormed)) {
					...
					if ((color == nextColor) && (color != Counter.EMPTY)) {
						...
						if (tilesCounter == Resources.TILES_TO_WIN) {
							isFormed = true;
							// finished = true; En conecta cuatro no usamos esto
							this.winner = color;
						}
					}
					else
